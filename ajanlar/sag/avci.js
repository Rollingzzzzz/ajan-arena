function yonBul(durum) {
  ...
  function avciCekirdek() {
    ...
    function tuket(start, basYon, mod) {
      ...
      while (adim < 1400) {
        ...
        for (let k = 0; k < 4; k++) {
          ...
          else if (mod === 2) s = derece * 4 + Math.random() * 3;
          else s = (4 - derece) * 6 + (d === yon ? 2 : 0);
          if (derece === 0) s -= 1000;
    ...
    function cepUzunluk(start, basYon) {
      ...
      if (say <= 30) { dfsDugum = 0; dfsZiy.fill(0); return dfsUzun(start); }
    ...
    function degerlendir(cI, cD, rI, rD) {
      ...
      if (bagli && benimSay < 26) return -9e5 + benimSay * 1000 - Math.min(mesafe, 20) * 5;
