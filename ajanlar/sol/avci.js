...
var mj = mx + my * W;
if (temel[mj]) continue;
var cak2 = 0, k4;
for (k4 = 0; k4 < rakipHamle.length; k4++) if (rakipHamle[k4] === mj) { cak2 = 1; break; }
if (!cak2) { aday.push({ d: d2, j: mj, x: mx, y: my }); break; }
if (!yedek) yedek = { d: d2, j: mj, x: mx, y: my };
}
if (!aday.length && yedek) aday.push(yedek);
}
if (!aday.length) {
for (i = 0; i < 4; i++) {
var d3 = YONLER[i];
...
var avAktif = ortak === 1 && mesafeKafa <= 5 && alanBen > alanRakip * 1.8;
var enSkor = -1e9;
...
if (cokYakin) skor -= 8;
var yeniMes = Math.abs(c.x - r.x) + Math.abs(c.y - r.y);
if (!avAktif && yeniMes <= 4) skor -= (5 - yeniMes) * 1.5;
}
